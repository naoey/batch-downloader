...
module.exports = {
  ...
  throttleDownloads: function(downloads, limit) {
    console.info(`Downloading ${downloads.length} items with limit ${limit}`);
    var i = 0;
    var resultArray = new Array(downloads.length);

    function next() {
      if (i < downloads.length) {
        var currentIdx = i++;
        var nextDownload = downloads[currentIdx];

        return Promise.resolve(nextDownload())
        .then(function(result) {
          resultArray[currentIdx] = result;
          return;
        })
        .then(next);
      }
    ...
    while(i < limit && i < downloads.length) {
      listOfDownloads.push(next());
    }

    return Promise.all(listOfDownloads).then(() => resultArray);
  }
};
