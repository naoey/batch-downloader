...
var downloadVid = function(url) {
  //TODO: Skip video and download only audio
...
var extractAudio = function(data) {
  return new Promise(function(resolve, reject) {
    var outputname = program.outputDir + data.title.replace('/', '') + ".mp3";
    ...
    exec(cmd, { maxBuffer: 1024 * 500 }, function(err, stdout, stderr) {
      if (err)
        ...
        resolve({source: data.tmpname, result: outputname});
      }
    );
  });
};
...
module.exports = function(url) {
  ...
  return new Promise(function(resolve, reject) {
    utils.log("Starting download of " + url).then(function() {
      ...
      if (cleanRes)
        utils.log("Cleaned temp file");
      else
        utils.log("Error cleaning temp file");
      resolve();
    });
  });
};
